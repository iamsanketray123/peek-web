    ...
    <div className="mx-auto max-w-5xl px-6 py-8">
      ...
      <form onSubmit={handleAdd} className="mb-6 flex flex-col gap-2 sm:flex-row">
        ...
        <select
          ...
          className="rounded-xl border border-line bg-surface px-3 py-2.5 text-sm outline-none focus:border-lime/50"
        >
          {COUNTRIES.map((c) => (
        ...
        <button
          type="submit"
          disabled={pending}
          className="flex items-center justify-center gap-2 rounded-xl bg-lime px-5 py-2.5 text-sm font-semibold text-ink transition hover:bg-lime-dim disabled:opacity-40"
        >
      ...
      {apps.length === 0 ? (
        <div className="rounded-2xl border border-dashed border-line bg-surface/40 px-6 py-16 text-center">
          ...
          <p className="mt-3 text-sm font-medium">No apps tracked yet</p>
          <p className="mt-1 text-sm text-muted">
        ...
        <ul className="space-y-2">
          {apps.map((app) => (
            <li
              key={app.id}
              className="group flex items-center gap-3 rounded-2xl border border-line bg-surface px-4 py-3 transition hover:border-lime/30"
            >
              ...
              <button
                ...
                className="rounded-lg p-2 text-faint opacity-0 transition hover:bg-surface-3 hover:text-red-300 group-hover:opacity-100 disabled:opacity-40"
                title="Remove app"
              >
                {removingId === app.id ? (
